...
bool BleOtaLib::begin(const std::string& deviceName, OTAStorage& storage, const BleOtaInfo& info, bool uploadEnable) {
...
bool BleOtaLib::begin(BLEServer* server, OTAStorage& storage, const BleOtaInfo& info, bool uploadEnable) {
    ...
    auto* rxCharacteristic = service->createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_RX, NIMBLE_PROPERTY::WRITE_NR);
    rxCharacteristic->setCallbacks(this);

    auto* txCharacteristic =
        service->createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_TX, NIMBLE_PROPERTY::READ | NIMBLE_PROPERTY::NOTIFY);
...
void BleOtaLib::begin(BLEService& service, const BleOtaInfo& info) {
    auto* mfNameCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_MF_NAME, NIMBLE_PROPERTY::READ);
    mfNameCharacteristic->setValue(reinterpret_cast<const uint8_t*>(info.mfName), strlen(info.mfName));
    auto* hwNameCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_HW_NAME, NIMBLE_PROPERTY::READ);
    hwNameCharacteristic->setValue(reinterpret_cast<const uint8_t*>(info.hwName), strlen(info.hwName));
    auto* swNameCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_SW_NAME, NIMBLE_PROPERTY::READ);
    swNameCharacteristic->setValue(reinterpret_cast<const uint8_t*>(info.swName), strlen(info.swName));
    auto* hwVerCharacteristic = service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_HW_VER, NIMBLE_PROPERTY::READ);
    hwVerCharacteristic->setValue(reinterpret_cast<const uint8_t*>(&info.hwVersion), sizeof(BleOtaVersion));
    auto* swVerCharacteristic = service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_SW_VER, NIMBLE_PROPERTY::READ);
