...
void BleOtaLib::begin(const std::string& deviceName, OTAStorage& storage, const BleOtaInfo& info, bool uploadEnable) {
...
void BleOtaLib::begin(BLEServer* server, OTAStorage& storage, const BleOtaInfo& info, bool uploadEnable) {
    ...
    auto* rxCharacteristic =
        service->createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_RX, BLECharacteristic::PROPERTY_WRITE_NR);
    rxCharacteristic->setCallbacks(this);

    auto* txCharacteristic = service->createCharacteristic(
        BLE_OTA_CHARACTERISTIC_UUID_TX, BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_NOTIFY);
...
void BleOtaLib::begin(BLEService& service, const BleOtaInfo& info) {
    auto* mfNameCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_MF_NAME, BLECharacteristic::PROPERTY_READ);
    mfNameCharacteristic->setValue(info.mfName);
    auto* hwNameCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_HW_NAME, BLECharacteristic::PROPERTY_READ);
    hwNameCharacteristic->setValue(info.hwName);
    auto* swNameCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_SW_NAME, BLECharacteristic::PROPERTY_READ);
    swNameCharacteristic->setValue(info.swName);
    auto* hwVerCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_HW_VER, BLECharacteristic::PROPERTY_READ);
    hwVerCharacteristic->setValue((uint8_t*)&info.hwVersion, sizeof(BleOtaVersion));
    auto* swVerCharacteristic =
        service.createCharacteristic(BLE_OTA_CHARACTERISTIC_UUID_SW_VER, BLECharacteristic::PROPERTY_READ);
